...
class Discrepancy_loss(Layer):
    def __init__(self, batch_size, **kwargs):
        super(Discrepancy_loss, self).__init__(**kwargs)
        self.batch_size = batch_size
...
class Entropy_loss(Layer):
    def __init__(self, batch_size, **kwargs):
        super(Entropy_loss, self).__init__(**kwargs)
        self.batch_size = batch_size
    ...
    def call(self, x, mask=None):
        ...
        loss_a = K.mean(K.categorical_crossentropy(a, a), axis=-1)
        loss_b = K.mean(K.categorical_crossentropy(b, b), axis=-1)
